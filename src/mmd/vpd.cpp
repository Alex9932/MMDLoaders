...
#define streql Engine::Helper::streql
#define strstw Engine::Helper::strstw
...
static Sint32 getBoneID(char* name) {
	...
	MMD_Utils::SJISToUTF8(sjis_name);

//	printf("~vpd name: %s\n", name);

	COPY_STRING(name, MMD_Utils::GetBuffer(), SDL_strlen(MMD_Utils::GetBuffer()));
...
vpd_pose* vpd_fromFile(const char* file) {
	...
	if(!streql(vpd, "Vocaloid Pose Data")) {
		printf("VPD: %s is not Vocaloid Pose Data file!\n", file);
		fclose(f);
		return NULL;
	}
	...
	while(readLine(f)) {
		...
		if(LINE_BUFFER[0] != '\n') {
			if(strstw(LINE_BUFFER, "Bone")) {
				...
				if(bone == -1) {
					printf("VPD: Parse error! Line[%d]: %s", line, LINE_BUFFER);
					return NULL;
				}
